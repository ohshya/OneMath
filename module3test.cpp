...
{
    QString style = QString("#nextPart{image: url(:/buttons/buttons/%1ok.png);}").arg(global::m3points);
...
void module3test::update(){
    ...
    limpiar();
    if(testCase!=0){
        QPixmap image(QString(":/funciones/graph%1.png").arg(testCase));
        ui->funcion->setPixmap(image);
        ui->funcion->setScaledContents(true);
        ui->lim1->setPixmap(QString(":/funciones/lim%1.png").arg(testCase));
    }else{
